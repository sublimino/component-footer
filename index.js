...
function renderSocialListContent(array) {
  return array.map(function (item) {
    var className = ['icon', 'icon--' + item.type + '-london', 'ec-footer__link'];
    return _react2['default'].createElement(
      'a',
      { href: item.href, title: item.title, className: className.join(' ') },
      item.text
    );
  });
}
...
var Footer = (function (_React$Component) {
  ...
  Footer.prototype.render = function render() {
    ...
    var quote = _react2['default'].createElement('p', { dangerouslySetInnerHTML: html });
    /*eslint-enable */
